<?php
    class crud{
        ...
        {
            $data=array();
            $p=crud::conect()->prepare('SELECT * FROM crudsystem');
            $p->execute();
        ...
        {
            $p=crud::conect()->prepare('DELETE FROM crudsystem WHERE id=:id');
            $p->bindValue(':id',$id);
            $p->execute();
        }
...
{
    $data=array();
    $p=crud::conect()->prepare('SELECT * FROM crudsystem WHERE id=:id');
    $p->bindValue(':id',$id);
    $p->execute();
